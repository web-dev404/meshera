...
export const OneTourItem: 
  ...
  ({ title, stars, href, description, className, times }) => {

  return (
    <article className={clsx(styles.onetour, className)}>
      <div className={styles.onetour__action}>
        <Link href={href} className={styles.onetour__link}>{title}</Link>
      </div>
      <div className={styles.onetour__body}>
        <div className={styles.onetour__stars}>
          {
            stars.map((isStar, _id) => (
              <span key={_id} className={clsx(styles.onetour__star, { [styles.onetour__star_active]: isStar }, "_icon-star-main")} />
            ))
          }
        </div>
